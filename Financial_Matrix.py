...
def generate_graph(principal, annual_contribution, min_rate, max_rate,
    ...
    try:
        ...
        print("A_nominal shape:", A_nominal.shape)
        print("real_A shape:", real_A.shape)

        print("T sample:", T[:5, :5])
        print("R sample:", R[:5, :5])
        print("A_nominal sample:", A_nominal[:5, :5])
        print("real_A sample:", real_A[:5, :5])

        # Flatten the arrays for Scatter3d
        T_flat = T.flatten()
        R_flat = (R * 100).flatten()  # Convert 0.00..0.15 -> 0..15
        A_nominal_flat = A_nominal.flatten()
        real_A_flat = real_A.flatten()
        ...
        fig = go.Figure(data=[
            go.Scatter3d(
                ...
                marker=dict(
                    size=5,
                    color=A_nominal_flat,
                    colorscale='Viridis',
                    colorbar=dict(title='Nominal Balance ($)')
                ),
                customdata=real_A_flat,
                hovertemplate=(
                    "Time: %{x} years<br>"
                    "Growth Rate: %{y:.2f}%<br>"
